...
std::vector<Eigen::Matrix3d> CAM_RIC;
std::vector<Eigen::Vector3d> CAM_TIC;
std::vector<Eigen::Matrix3d> LASER_RIC;
std::vector<Eigen::Vector3d> LASER_TIC;
...
std::string LASER_FEATURE_TOPIC;
...
void readParameters(ros::NodeHandle &n)
{
    ...
    if (ESTIMATE_EXTRINSIC == 2)
    {
        ...
        CAM_RIC.push_back(Eigen::Matrix3d::Identity());
        CAM_TIC.push_back(Eigen::Vector3d::Zero());
    ...
    else
    {
        ...
        CAM_RIC.push_back(eigen_R);
        CAM_TIC.push_back(eigen_T);
        ROS_INFO_STREAM("Extrinsic_R : " << std::endl << CAM_RIC[0]);
        ROS_INFO_STREAM("Extrinsic_T : " << std::endl << CAM_TIC[0].transpose());
